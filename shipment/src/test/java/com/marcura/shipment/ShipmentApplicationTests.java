...
class ShipmentApplicationTests {
    ...
    @MockBean
    private Worker createShipmentWorker;

    @Test
    public void testShipment_success() {
        when(shipmentService.ship(any(OrderDto.class))).thenReturn("1234-SHIP-REF");
        String result = shipmentService.ship(generateDto());
        Assertions.assertEquals("1234-SHIP-REF", result);
    }
